...
class AccountBankStatementImport(models.TransientModel):
    ...
    journal_id = fields.Many2one(
        ...
        help="Accounting journal related to the bank statement "
             "you're importing.")
    ...
    def _find_additional_data(self, *args):
        """ As .CBI format does not allow us to detect the journal, we need
            to let the user choose it. We set it in context in the same way
            it's done when calling the import action from a journal.
        """
        if self.journal_id:
    ...
    def _parse_file(self, data_file):
        ...
        vals_bank_statement = {}
        flow = Flow()
        flow.readfile(data_file.decode('utf-8').split("\n"),
        ...
        for disposal in flow.disposals:
            for record in mergeColumnRecord(disposal.records,
                ...
                if BankStatementLine.sudo().search_count(
                        [('unique_import_id', '=', unique_val)]) > 0:
                    logging.warning("Row Already in the database %r" %
                                    unique_val)
                    continue
                ...
                vals_line['date'] = datetime.datetime.strptime(
                    record['data'], "%d%m%y").strftime(
        ...
        print("vals_bank_statement: {}".format(vals_bank_statement))
