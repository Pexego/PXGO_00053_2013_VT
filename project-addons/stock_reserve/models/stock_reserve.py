...
class stock_reservation(models.Model):
    ...
    * note
    ...
    # ~ def _default_location_id(self):
        ...
        # ~ return move_obj.with_context(picking_type='internal').\
            # ~ _default_location_source()

    @api.model
    def _default_location_dest_id(self):
        return self.env.ref('stock_reserve.stock_location_reservation')

    sequence = fields.Integer('Sequence', default=_new_sequence,
                              help="Gives the priority in reservation.")
    ...
    #TODO: Migrarlocation_id = fields.Many2one(default=_default_location_id)
    ...
    def do_complete_release(self, cr, uid, ids, context=None):
        ...
        for reserve in self.browse(cr, uid, ids, context=context):
            if reserve.state in ('done', 'cancel'):
                raise exceptions.UserError(_('Cannot unreserve a done reserve'))
