...
class RappelListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=10, eta=60).export_rappel()

    def on_record_write(self, record, fields=None):
        up_fields = ["name"]
        for field in up_fields:
            if field in fields:
                record.with_delay(priority=10, eta=120).update_rappel(fields=fields)

    def on_record_unlink(self, record):
        record.with_delay(priority=10, eta=120).unlink_rappel()
...
class RappelInfoListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        if record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=10).export_rappel_info()

    def on_record_write(self, record, fields=None):
        if record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=10, eta=60).update_rappel_info(fields=fields)

    def on_record_unlink(self, record):
        if record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=10, eta=60).unlink_rappel_info()
...
class RappelSectionListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=10).export_rappel_section()

    def on_record_write(self, record, fields=None):
        record.with_delay(priority=10, eta=60).update_rappel_section(fields=fields)

    def on_record_unlink(self, record):
        record.with_delay(priority=10, eta=60).unlink_rappel_section()
