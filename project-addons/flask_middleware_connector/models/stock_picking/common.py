...
class StockMoveListener(Component):
    ...
    def on_stock_move_change(self, record):
        record._cr.execute("select trim(trailing ']' from trim(leading '[' from record_ids))::int from queue_job "
                           "where state = 'pending' and job_function_id = 486 and model_name = 'product.product'")
        res = record._cr.fetchall()
        products = [x for x in map(lambda l: l[0], res)]

        if record.product_id.show_stock_outside and record.product_id.id not in products:
        ...
        for pack in packs:
            if pack.product_tmpl_id.product_variant_ids.id not in products:
