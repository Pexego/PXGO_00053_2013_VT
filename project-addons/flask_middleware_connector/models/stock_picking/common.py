...
class PickingListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        ...
        if picking.partner_id.commercial_partner_id.web \
            ...
            picking.with_delay(priority=10, eta=30).export_picking()
        ...
        if picking.partner_id.commercial_partner_id.web \
            ...
            if 'name' in fields or 'partner_id' in fields or ('not_sync' in fields and not picking.not_sync):
                ...
                for product in picking.move_lines:
                    product.with_delay(priority=10, eta=60).export_pickingproduct()
            elif picking.state == 'cancel' \
                    or ('not_sync' in fields and picking.not_sync) \
                    or ('company_id' in fields and picking.company_id != 1):
                for product in picking.move_lines:
                    product.with_delay(priority=10, eta=30).unlink_pickingproduct()
                picking.with_delay(priority=10, eta=60).unlink_picking()
            else:
                if set(fields).intersection(set(up_fields)):
                    picking.with_delay(priority=10, eta=30).update_picking(fields=fields)
        else:
            ...
            if job and 'unlink' not in job.name:
                record.with_delay(priority=10, eta=30).unlink_picking()
    ...
    def on_record_unlink(self, record):
        ...
        if picking.partner_id.commercial_partner_id.web \
                ...
                and picking.company_id.id == 1:
            for product in picking.move_lines:
                product.with_delay(priority=10, eta=120).unlink_pickingproduct()
            record.with_delay(priority=10, eta=120).unlink_picking()
    ...
    def on_record_create(self, record, fields=None):
        for move_line in record:
            if move_line.picking_id.partner_id.commercial_partner_id.web \
                ...
                record.with_delay(priority=10, eta=60).export_pickingproduct()
        ...
        for move_line in record:
            if move_line.picking_id.partner_id.commercial_partner_id.web \
                ...
                if 'picking_id' in fields:
                    record.with_delay(priority=10, eta=60).export_pickingproduct()
                if set(fields).intersection(set(up_fields)):
                    record.with_delay(priority=10, eta=120).update_pickingproduct(fields=fields)
    ...
    def on_record_unlink(self, record):
        for move_line in record:
            if move_line.picking_id.partner_id.commercial_partner_id.web \
                    ...
                    and not move_line.picking_id.not_sync \
                    and move_line.picking_id.company_id.id == 1:
                record.with_delay(priority=10, eta=120).unlink_pickingproduct()
