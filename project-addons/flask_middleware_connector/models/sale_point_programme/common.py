...
class SalePointProgrammeRuleListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=10, eta=60).export_rule()

    def on_record_write(self, record, fields=None):
        up_fields = ["name","points","value","product_category_id","product_brand_id","product_id","operator","date_end"]
        for field in up_fields:
            if field in fields:
                record.with_delay(priority=10, eta=120).update_rule(fields=fields)

    def on_record_unlink(self, record):
        record.with_delay(priority=10, eta=120).unlink_rule()
...
class ResPartnerPointProgrammeBagAccumulatedListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        if record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=10, eta=60).export_point_programme_bag_acc()

    def on_record_write(self, record, fields=None):
        if record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=10, eta=120).update_point_programme_bag_acc(fields=fields)

    def on_record_unlink(self, record):
        if record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=10, eta=120).unlink_point_programme_bag_acc()
