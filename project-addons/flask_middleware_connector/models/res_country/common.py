...
class ResCountryListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=10).export_country()

    def on_record_write(self, record, fields=None):
        up_fields = ["name", "code"]
        for field in up_fields:
            if field in fields:
                record.with_delay(priority=10).update_country(fields=fields)
                break

    def on_record_unlink(self, record):
        record.with_delay(priority=10).unlink_country()
...
class CountryStateListener(Component):
    ...
    def on_record_write(self, record, fields=None):
        ...
        for field in up_fields:
            if field in fields:
                record.with_delay(priority=10).update_country_state(fields=fields)
                break

    def on_record_unlink(self, record):
        record.with_delay(priority=10).unlink_country_state()
