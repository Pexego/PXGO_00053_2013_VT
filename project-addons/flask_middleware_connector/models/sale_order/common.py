...
class SaleOrderListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        if record.partner_id.web or record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=11, eta=80).export_order()
    ...
    def on_record_write(self, record, fields=None):
        ...
        if record.partner_id.web or record.partner_id.commercial_partner_id.web:
            if 'state' in fields and record.state == 'cancel':
                record.with_delay(priority=11, eta=80).unlink_order()
            elif 'state' in fields and record.state == 'draft':
                record.with_delay(priority=11, eta=80).export_order()
                for line in record.order_line:
                    line.with_delay(priority=11, eta=100).export_orderproduct()
            elif record.state != 'cancel':
                for field in up_fields:
                    if field in fields:
                        record.with_delay(priority=11, eta=80).update_order(fields=fields)
                        break

    def on_record_unlink(self, record):
        if record.partner_id.web or record.partner_id.commercial_partner_id.web:
            record.with_delay(priority=11, eta=120).unlink_order()
...
class SaleOrder(models.Model):
    ...
    @job(retry_pattern={1: 10 * 60, 2: 20 * 60, 3: 30 * 60, 4: 40 * 60, 5: 50 * 60})
    def update_order(self, fields=None):
        backend = self.env["middleware.backend"].search([])[0]
        with backend.work_on(self._name) as work:
            exporter = work.component(usage='record.exporter')
...
class SaleOrderLineListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        if record.order_id.partner_id.web or record.order_id.partner_id.commercial_partner_id.web:
            record.with_delay(priority=11, eta=120).export_orderproduct()
    ...
    def on_record_write(self, record, fields=None):
        ...
        if record.order_id.partner_id.web or record.order_id.partner_id.commercial_partner_id.web:
            for field in up_fields:
                if field in fields:
                    record.with_delay(priority=11, eta=120).update_orderproduct(fields=fields)
                    break

    def on_record_unlink(self, record):
        if record.order_id.partner_id.web or record.order_id.partner_id.commercial_partner_id.web:
            record.with_delay(priority=11, eta=180).unlink_orderproduct()
