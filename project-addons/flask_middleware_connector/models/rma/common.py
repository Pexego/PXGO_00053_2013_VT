...
class CrmClaimListener(Component):
    ...
    def on_record_write(self, record, fields=None):
        ...
        if rma.partner_id and rma.partner_id.web and not rma.in_web:
            rma.in_web = True
            rma.with_delay(priority=5).export_rma()
            ...
            rma.in_web = False
            rma.with_delay(priority=5, eta=120).unlink_rma()
    ...
    def on_record_unlink(self, record):
        if record.partner_id and record.partner_id.web:
            record.in_web = False
            record.with_delay(priority=5, eta=120).unlink_rma()
...
class ClaimLineListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        if record.claim_id.partner_id.web and \
                ...
                record.web:
            record.with_delay(priority=5, eta=120).export_rmaproduct()
    ...
    def on_record_write(self, record, fields=None):
        ...
        if 'web' in fields and record.web:
            ...
            record.with_delay(priority=5, eta=180).unlink_rmaproduct()
        elif record.claim_id.partner_id.web and record.web:
            for field in up_fields:
                if field in fields:
                    record.with_delay(priority=5, eta=180).update_rmaproduct()
                    break
...
class SubstateListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=5).export_rma_status()

    def on_record_write(self, record, fields=None):
        up_fields = ["name"]
        for field in up_fields:
            if field in fields:
                record.with_delay(priority=5).update_rma_status()
                break

    def on_record_unlink(self, record):
        record.with_delay(priority=5).unlink_rma_status()
...
class ClaimStageListener(Component):
    ...
    def on_record_create(self, record, fields=None):
        record.with_delay(priority=5).export_rma_stage()

    def on_record_write(self, record, fields=None):
        up_fields = ["name"]
        for field in up_fields:
            if field in fields:
                record.with_delay(priority=5).update_rma_stage()
                break

    def on_record_unlink(self, record):
        record.with_delay(priority=5).unlink_rma_stage()
