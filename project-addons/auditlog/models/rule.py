...
class AuditlogRule(models.Model):
    ...
    not_trace_request = fields.Boolean("Not trace request")
    not_trace_session = fields.Boolean("Not trace session")
    name_method = fields.Char("Get name method",
                              help="If not set, name_get will be call, you "
                                   "can use 'o' to ref to the object")
    ...
    def create_logs(self, uid, res_model, res_ids, method,
        ...
        for res_id in res_ids:
            ...
            obj = model_model.browse(res_id)
            rules = self.env['auditlog.rule'].\
                search([('model_id', '=',
                         self.pool._auditlog_model_cache[res_model])])
            if not rules or not rules[0].name_method:
                name = obj.name_get()
                res_name = name and name[0] and name[0][1]
            else:
                res_name = eval(rules[0].name_method, locals_dict={'o': obj})
            vals = {
                'name': res_name,
                ...
                'user_id': uid}
            if rules:
                if not rules[0].not_trace_request:
                    vals['http_request_id'] = \
                        http_request_model.current_http_request()
                if not rules[0].not_trace_session:
                    vals['http_session_id'] = \
                        http_session_model.current_http_session()
