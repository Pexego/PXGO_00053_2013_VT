...
class TntRequest(object):
    ...
    @staticmethod
    def format_duration(float_field):
        return str(timedelta(seconds=float_field * 60 * 60))

    def _prepare_collection(self, partner):
        address = self._prepare_address(partner)
        del address["ACCOUNT"]
        shipdate = fields.Datetime.from_string(self.record.scheduled_date)
        return {
            "COLLECTION": {
                "COLLECTIONADDRESS": address,
                "SHIPDATE": "%s/%s/%s" % (shipdate.day, shipdate.month, shipdate.year),
                "PREFCOLLECTTIME": {
                    "FROM": self.format_duration(self.carrier.tnt_collect_time_from),
                    "TO": self.format_duration(self.carrier.tnt_collect_time_to),
                },
                "COLLINSTRUCTIONS": "",
            }
        }
    ...
    def _prepare_label_data(self):
        ...
        return {
            "consignment": {
                ...
                "collectionDateTime": fields.Datetime.from_string(fields.Datetime.now()).isoformat('T'),
