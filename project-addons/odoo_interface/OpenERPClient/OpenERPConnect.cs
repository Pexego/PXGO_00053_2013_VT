...
namespace OpenERPClient
{
	...
	public class OpenERPConnect
    {
		...
		public long Create(string model, XmlRpcStruct fieldValues)
		{
			...
			long new_id = this.rpcclient.Create(this.dbname, this.uid, this.password, model, "create", fieldValues);
			this.Close();
			return new_id;
		}
		...
		{
			this.Open(OpenERPClient.OpenERPService.Object);	
			var ids_obj = this.rpcclient.Search(this.dbname, this.uid, this.password, model, "search", filters);
			this.Close();
		...
		{
			this.Open(OpenERPClient.OpenERPService.Object);
			var data = this.rpcclient.Read (this.dbname, this.uid, this.password, model, "read", ids, fields);
			this.Close();
