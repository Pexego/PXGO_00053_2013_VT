...
class sale_equivalent_products(orm.TransientModel):
    ...
    def _get_products(self, cr, uid, ids, field_name, arg, context=None, tag_ids=None, onchange=None):
        res = {}
    ...
    def onchange_tags(self, cr, uid, ids, tag_ids=False, context=None):
        if not tag_ids:
            return True
        tag_ids = tag_ids[0][2]
        product_ids = self._get_products(cr, uid, ids, "", "", context, tag_ids, True)[ids[0]]
        return {'value': {'product_ids': product_ids}, 'domain': {'product_id': [('id', 'in', product_ids)]}}
    ...
    def select_product(self, cr, uid, ids, context=None):
        ...
        if wiz.product_id.id not in [x.id for x in wiz.product_ids]:
            raise orm.except_orm(_('Error'), _('El producto no es equivalente'))
        self.pool.get('sale.order.line').write(cr, uid, wiz.line_id.id, {'product_id': wiz.product_id.id}, context)
        self.pool.get('sale.order.line').product_id_change(cr, uid, wiz.line_id.id, wiz.line_id.order_id.pricelist_id.id, wiz.product_id.id, wiz.line_id.product_uom_qty, False, wiz.line_id.product_uos_qty, False, wiz.line_id.name, wiz.line_id.order_id.partner_id, False, True, wiz.line_id.order_id.date_order, False, wiz.line_id.order_id.fiscal_position, False, context)
