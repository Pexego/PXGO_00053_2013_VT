...
class import_invoice(object):
    def __init__(self, dbname, user, passwd, invoices):
        """método incial"""
    ...
    def import_invoices(self):
        cwb = xlrd.open_workbook(self.invoices, encoding_override="utf-8")
        ...
        for rownum in range(1, all_lines):
            ...
            try:
                invoice_ids = self.search("account.invoice", [('number', '=', str(int(record[0])))])
                if not invoice_ids:
                    last_number = str(int(record[0]))
                    ...
                    vals = {'odoo_id': invoice.id,
                            ...
                            'total_wt_rect': invoice.total_wt_rectflas}
                    ...
                    invoice_id = self.create("account.invoice", vals)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        ...
        import_invoice(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
