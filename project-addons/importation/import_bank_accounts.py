...
class import_banks(object):
    def __init__(self, dbname, user, passwd, accounts_file, payment_mode_id_mandate=False):
        """método incial"""
            ...
            try:
                ...
                if partner_ids:
                    ...
                    if self.payment_mode_id_mandate:
                        partner_data = self.read("res.partner", partner_ids[0],
                                                 ['customer_payment_mode',
                                                  'supplier_payment_mode'])
                        if partner_data['customer_payment_mode'] and \
                                partner_data['customer_payment_mode'][0] == \
                                self.payment_mode_id_mandate:
                            receipt = True
                        elif partner_data['supplier_payment_mode'] and \
                                partner_data['supplier_payment_mode'][0] == \
                                self.payment_mode_id_mandate:
                            receipt = True

                    bank_ids = self.search("res.bank", [('code', '=',
                                                         record[4][4:8])])
                    country_ids = self.search("res.country", [('code', '=',
                                                               record[4][:2])])
                    bank_vals = {
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print u"Uso: %s <dbname> <user> <password> <accounts.xls> <payment_mode_id_mandate (optional)>" % sys.argv[0]
    else:
        import_banks(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], len(sys.argv) == 6 and sys.argv[5] or False)
