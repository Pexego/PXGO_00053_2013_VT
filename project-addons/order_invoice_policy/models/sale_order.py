        ...
        for order in self:
            if order.state not in ('sale', 'done'):
                ...
                if any(line.invoice_status == 'to invoice'
                       for line in order.order_line.filtered(lambda p: p.product_id.type in ['product', 'consu'])) \
                        or (order.order_line and all(line.invoice_status == 'to invoice' and line.product_id.type == 'service'
